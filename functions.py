...
class CEC_functions:
    ...
    def Y(self , X , fun_num, rflag = None):
        ...
        # 2       
            ...
            self.T_osz(X_rotate)
            Y = np.sum((1e6 ** (np.arange(d)/(d-1))) * X_rotate**2) 
        ...
        # 4
            ...
            X_rotate = self.M1 @ X_shift
            self.T_osz(X_rotate)
            Y = (1e6) * (X_rotate[0]**2) + np.sum(X_rotate[1:d]**2) 
        ...
        # 5
            ...
            if rf == 1:
                Z = self.M1 @ Z
            Y = np.sqrt(np.sum(abs(Z)**(2 + (4 * np.arange(d)/(d-1)).astype(int)))) 
        ...
        # 6
            ...
            Z = X_rotate + 1
            Y = np.sum (100 * (Z[:d-1]**2 - Z[1:d])**2 + (Z[:d-1] - 1)**2) 
        ...
        # 7
            ...
            Y = (np.sum(np.sqrt(Z) + np.sqrt(Z) * np.sin(50 * Z**0.2)**2 ) / (d-1))**2 
        ...
        # 9
            ...
            Y = np.sum([_w(Z[i] + 0.5) for i in range(d)]) - (_w(0.5) * d) 
        ...
        # 10
            ...
            Y = 1.0 + (np.sum(Z**2 )/4000.0) - np.multiply.reduce(np.cos(Z / np.sqrt(np.arange(1, d + 1)))) 
        ...
        # 11
            ...
            Y = np.sum(10 + Z**2 - 10 * np.cos(2 * np.pi * Z)) 
        ...
        # 14 15    
            ...
            Z[Z < -500]     = \
                ...
                - (Z[Z < -500] + 500)**2 / (10000*d)
            Y = 418.9828872724338 * d - np.sum(Z) 
        ...
        # 17 18
            ...
            Y   = Y_3 + 10 * (d - np.sum(np.cos(2 * np.pi * Z))) 
        ...
        # 20
            ...
            Y = np.sum(0.5 + (np.sin(np.sqrt(tmp1))**2 - 0.5)/(1 + 0.001 * tmp1)**2) 
        ...
        # 21
            ...
            Y= self.cf_cal(X, delta, bias, np.array(fit)) 
            ...
            Y = Y= self.cf_cal(X, delta, bias, np.array(fit)) 
